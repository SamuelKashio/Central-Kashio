...
def obtener_estado_actual_agente(df_sal, agent_id):
    ...
    if str(ult["numero_cliente"])=="*34":
        mins=int((now_lima()-ult["detect_time"]).total_seconds()/60)
        return {"estado":"Online","desde":ult["detect_time"],"duracion":f"{mins//60}h {mins%60}m"}
    return {"estado":"Offline","desde":ult["detect_time"],"duracion":"—"}

def calcular_metricas_supervisor(df_sal, agent_id):
    if df_sal is None or df_sal.empty: return {}
    hoy=now_lima().date()
...
tabs=st.tabs(["VISIÓN GENERAL","ENTRANTES","SALIENTES","AGENTES","TURNOS","SEGUIMIENTO","CLIENTES","SUPERVISIÓN","REGISTROS"])
tab_ov,tab_ent,tab_sal,tab_ag,tab_tur,tab_seg,tab_cl,tab_sup,tab_raw_t=tabs
...
with tab_sup:
    st.markdown(f"<h3 style='color:{c['text']}'>👤 Supervisión de Agentes</h3>",unsafe_allow_html=True)
    ev_todos = detectar_eventos_login_logout(df_sal)
    s1,s2,s3 = st.tabs(["Estado Actual","Métricas Hoy","Historial"])

    with s1:
        st.markdown(f"<p style='color:{c['muted']};font-size:12px;letter-spacing:1px'>CONEXIÓN EN TIEMPO REAL</p>",unsafe_allow_html=True)
        agentes_activos = {aid:info for aid,info in st.session_state.cfg_agentes.items() if not info.get("es_central") and info.get("activo")}
        cols_sup = st.columns(3)
        for idx,(aid,info) in enumerate(sorted(agentes_activos.items())):
            est = obtener_estado_actual_agente(df_sal,aid)
            online = est["estado"]=="Online"
            color_borde = c["green_border"] if online else c["red_border"] if est["estado"]=="Offline" else c["border"]
            color_dot   = c["green"]        if online else c["red"]        if est["estado"]=="Offline" else c["muted"]
            desde_txt   = est["desde"].strftime("%H:%M") if est["desde"] else "—"
            dur_txt     = est["duracion"]
            estado_txt  = est["estado"]
            with cols_sup[idx%3]:
                st.markdown(f"""<div style='background:{c["card"]};border:1px solid {color_borde};border-radius:12px;padding:18px;margin-bottom:12px;text-align:center'>
                    <div style='width:12px;height:12px;border-radius:50%;background:{color_dot};margin:0 auto 10px'></div>
                    <div style='color:{c["text"]};font-weight:600;font-size:14px;margin-bottom:6px'>{info["nombre"]}</div>
                    <div style='color:{color_dot};font-size:12px;font-weight:600;margin-bottom:8px'>{estado_txt}</div>
                    <div style='color:{c["muted"]};font-size:11px'>Desde: {desde_txt}</div>
                    <div style='color:{c["muted"]};font-size:11px'>Duración: {dur_txt}</div>
                </div>""",unsafe_allow_html=True)

    with s2:
        st.markdown(f"<p style='color:{c['muted']};font-size:12px;letter-spacing:1px'>MÉTRICAS DEL DÍA</p>",unsafe_allow_html=True)
        rows=[]
        for aid,info in sorted(st.session_state.cfg_agentes.items()):
            if info.get("es_central"): continue
            est = obtener_estado_actual_agente(df_sal,aid)
            metr = calcular_metricas_supervisor(df_sal,aid)
            if not metr: continue
            dot = "🟢" if est["estado"]=="Online" else "🔴" if est["estado"]=="Offline" else "⚪"
            rows.append({"":dot,"Agente":info["nombre"],"Llamadas":metr["llamadas_totales"],"Atendidas":metr["llamadas_atendidas"],"Dur.Prom":f"{metr['duracion_promedio']}s","Conectado":f"{metr['tiempo_conectado_minutos']}m","c/h":metr["productividad"]})
        if rows:
            st.dataframe(pd.DataFrame(rows),use_container_width=True,hide_index=True,height=300)
        else:
            st.info("Sin datos de supervisión por ahora")

    with s3:
        st.markdown(f"<p style='color:{c['muted']};font-size:12px;letter-spacing:1px'>HISTORIAL LOGIN / LOGOUT — HOY</p>",unsafe_allow_html=True)
        if not ev_todos.empty:
            hoy_s=ev_todos[ev_todos["detect_time"].dt.date==now_lima().date()].copy()
            if not hoy_s.empty:
                hoy_s["Hora"]=hoy_s["detect_time"].dt.strftime("%H:%M:%S")
                df_hist=hoy_s[["Hora","agente","tipo_evento","numero_cliente"]].rename(columns={"agente":"Agente","tipo_evento":"Evento","numero_cliente":"Código"})
            ...
            st.info("Sin datos — los agentes aún no han marcado *34 o *33")
